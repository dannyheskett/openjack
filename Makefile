...
# OPENJACK_VERSION explicitly; for local `make dist` it derives from the
...
# repo's release.yml is byte-identical. OPENJACK_VERSION still works as an explicit
...
OPENJACK_VERSION ?= $(RELEASE_VERSION)
VERSION_SLUG       := build-$(OPENJACK_VERSION)
...
OUT         := build/openjack
OUT_RELEASE := build/openjack-release
...
OUT_WIN64 := build/openjack-x64.exe
OUT_WIN32 := build/openjack-x86.exe
...
# native_app_glue into libopenjack.so, then package + sign an APK with
...
# only exists when this flag is present. Without it, dlopen of libopenjack.so
...
ANDROID_LIB      := $(ANDROID_APK_DIR)/lib/$(ANDROID_ABI)/libopenjack.so
ANDROID_APK      := build/openjack.apk
ANDROID_KEYSTORE ?= build/debug.keystore

# One tiny Java class (OpenjackActivity, a NativeActivity subclass that hides
...
ANDROID_JAVA_SRC := android/java/com/danheskett/openjack/OpenjackActivity.java
...
$(ANDROID_DEX): $(ANDROID_JAVA_SRC)
	...
	$(ANDROID_SDK_BT)/d8 --min-api $(ANDROID_API) --lib $(ANDROID_JAR) \
	    --output build/dex build/java-classes/com/danheskett/openjack/*.class
...
$(ANDROID_KEYSTORE):
	...
	keytool -genkeypair -keystore $@ -storepass android -keypass android \
	    -alias openjack -keyalg RSA -keysize 2048 -validity 10000 \
	    -dname "CN=openjack, O=openjack, C=US"
...
# sideload APK target: same libopenjack.so, different packaging + a real
...
# ---------------------------------------------------------------------------
ANDROID_AAB        := build/openjack.aab
...
$(ANDROID_AAB): $(ANDROID_LIB) $(ANDROID_DEX) $(BUNDLETOOL) $(PLAY_KEYSTORE) \
	...
	cp $(ANDROID_LIB) build/aab/module/lib/$(ANDROID_ABI)/libopenjack.so
...
# One adaptive layout serves a phone browser (touch) and a desktop browser
# (mouse + keyboard) alike; the loop is driven from emscripten_set_main_loop. The recorder is stubbed, so the
# encode_h264/mux TUs and minih264/minimp4 includes are dropped (as on Android).
# Outputs build/web/openjack.{html,js,wasm} with the shell in web/shell.html.
# ---------------------------------------------------------------------------
...
WEB_OUT     := $(WEB_OUT_DIR)/openjack.html
...
web-serve: $(WEB_OUT)
	@echo "openjack: http://localhost:8080/openjack.html"
...
# ---------------------------------------------------------------------------
IOS_MIN        ?= 15.0
IOS_APP_NAME   := Openjack
IOS_BUNDLE_ID  := com.danheskett.openjack
...
IOS_BUILD_NUMBER ?= $(OPENJACK_VERSION)
...
$(IOS_IPA): $(IOS_DEPS)
	...
	cd build/ios-device && rm -f ../openjack.ipa && zip -qr ../openjack.ipa Payload
	@if [ -n "$(IOS_SIGN_IDENTITY)" ]; then \
...
#                 rotation, and buttons that hit-test where they are drawn
...
dist-linux: release
	@rm -rf $(STAGING)/linux && mkdir -p $(STAGING)/linux/openjack-$(VERSION_SLUG) $(DIST)
	cp $(OUT_RELEASE) $(STAGING)/linux/openjack-$(VERSION_SLUG)/openjack
	cp $(DOCS) $(STAGING)/linux/openjack-$(VERSION_SLUG)/
	(cd $(STAGING)/linux && tar -czf ../../../$(DIST)/openjack-$(VERSION_SLUG)-linux-x86_64.tar.gz openjack-$(VERSION_SLUG))

dist-windows: $(OUT_WIN64) $(OUT_WIN32)
	@rm -rf $(STAGING)/win && mkdir -p $(DIST) \
	    $(STAGING)/win/openjack-$(VERSION_SLUG)-x64 \
	    $(STAGING)/win/openjack-$(VERSION_SLUG)-x86
	cp $(OUT_WIN64) $(STAGING)/win/openjack-$(VERSION_SLUG)-x64/openjack.exe
	cp $(DOCS) $(STAGING)/win/openjack-$(VERSION_SLUG)-x64/
	cp $(OUT_WIN32) $(STAGING)/win/openjack-$(VERSION_SLUG)-x86/openjack.exe
	cp $(DOCS) $(STAGING)/win/openjack-$(VERSION_SLUG)-x86/
	(cd $(STAGING)/win && zip -qr ../../../$(DIST)/openjack-$(VERSION_SLUG)-windows-x64.zip openjack-$(VERSION_SLUG)-x64)
	(cd $(STAGING)/win && zip -qr ../../../$(DIST)/openjack-$(VERSION_SLUG)-windows-x86.zip openjack-$(VERSION_SLUG)-x86)

dist-mac: $(OUT_MAC)
	@rm -rf $(STAGING)/mac && mkdir -p $(STAGING)/mac/openjack-$(VERSION_SLUG) $(DIST)
	cp $(OUT_MAC) $(STAGING)/mac/openjack-$(VERSION_SLUG)/openjack
	codesign --force --sign - $(STAGING)/mac/openjack-$(VERSION_SLUG)/openjack
	cp $(DOCS) $(STAGING)/mac/openjack-$(VERSION_SLUG)/
	(cd $(STAGING)/mac && zip -qr ../../../$(DIST)/openjack-$(VERSION_SLUG)-macos-universal.zip openjack-$(VERSION_SLUG))

# The APK is a self-contained installable, so it ships as-is (versioned name).
dist-android: $(ANDROID_APK)
	@mkdir -p $(DIST)
	cp $(ANDROID_APK) $(DIST)/openjack-$(VERSION_SLUG)-android-arm64.apk

# The Play upload artifact: the signed App Bundle (see the android-play target).
dist-android-play: $(ANDROID_AAB)
	@mkdir -p $(DIST)
	cp $(ANDROID_AAB) $(DIST)/openjack-$(VERSION_SLUG)-android.aab

dist-ios: $(IOS_IPA)
	@mkdir -p $(DIST)
	cp $(IOS_IPA) $(DIST)/openjack-$(VERSION_SLUG)-ios-arm64.ipa

# The web build ships as a zip of the HTML/JS/WASM (serve over http to play).
dist-web: $(WEB_OUT)
	@rm -rf $(STAGING)/web && mkdir -p $(STAGING)/web/openjack-$(VERSION_SLUG)-web $(DIST)
	cp $(WEB_OUT_DIR)/openjack.html $(WEB_OUT_DIR)/openjack.js $(WEB_OUT_DIR)/openjack.wasm \
	    $(STAGING)/web/openjack-$(VERSION_SLUG)-web/
	cp $(DOCS) $(STAGING)/web/openjack-$(VERSION_SLUG)-web/
	(cd $(STAGING)/web && zip -qr ../../../$(DIST)/openjack-$(VERSION_SLUG)-web-wasm.zip openjack-$(VERSION_SLUG)-web)
